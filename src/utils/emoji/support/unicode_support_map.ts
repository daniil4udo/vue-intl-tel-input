...
// Doing this for the typing sake
...
function checkPixelInImageDataArray(pixelOffset: number, imageDataArray) {
    // `4 *` because RGBA
...
const fontSize = 16;
function generateUnicodeSupportMap(testMap: IUnicodeSupportTestMap): IResultMap {
    const testMapKeys = Object.keys(testMap);
    const numTestEntries = testMapKeys.reduce((list, testKey) => list.concat(testMap[testKey]), []).length;
    ...
    const resultMap = testMapKeys.reduce((acc, testKey) => {
        ...
        const isTestSatisfied: boolean = []
            .concat(testEntry)
            .reduce(isSatisfied => {
                // Sample along the vertical-middle for a couple of characters
                const imageData = ctx.getImageData(0, (readIndex * fontSize) + (fontSize / 2), 2 * fontSize, 1).data;
        ...
        // acc[testKey] = isTestSatisfied;

        return Object.assign(acc, { [testKey]: isTestSatisfied });
    }, {} as IResultMap);

    return { ...resultMap, meta: { isChrome, chromeVersion } };
}
